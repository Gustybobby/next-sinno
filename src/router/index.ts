...
export type NextMethodHandlers<
  Req extends Request,
  NextParams extends BaseNextParams,
  MethodResults extends Record<string, unknown>,
> = {
  [method in keyof MethodResults]: method extends NextHTTPMethods
    ? NextMethodHandler<Req, NextParams>
    : never;
};
...
> = {
  [method in keyof MethodResults]: method extends NextHTTPMethods
    ? SinnoMethodHandler<Req, NextParams, Logger, MethodResults[method]>
    : never;
};
...
>(
...
): NextMethodHandlers<Req, NextParams, MethodResults> {
  return {
  ...
  } as NextMethodHandlers<Req, NextParams, MethodResults>;
}
